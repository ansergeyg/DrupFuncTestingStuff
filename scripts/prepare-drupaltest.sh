...
DRUPAL_CORE_DEV_CONSTRAINT="${DRUPAL_CORE_DEV_CONSTRAINT:-^10.5}"
...
if [ "$SKIP_DRUPALTEST_COMPOSER_REQUIRE" != "1" ]; then
  composer require --no-interaction --no-scripts --dev "drupal/core-dev:$DRUPAL_CORE_DEV_CONSTRAINT"
  composer require --no-interaction --no-scripts "drupal/paragraphs:$DRUPAL_PARAGRAPHS_CONSTRAINT"
fi
